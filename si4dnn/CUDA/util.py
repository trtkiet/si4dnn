...
def parse_torch_model(model: nn.Module):
    ...
    for module in model.modules():
        ...
        if isinstance(module, nn.BatchNorm1d):
            gamma = (
                torch.tensor(module.weight.detach(), device="cuda")
                if module.affine and module.weight is not None
                else None
            )
            beta = (
                torch.tensor(module.bias.detach(), device="cuda")
                if module.affine and module.bias is not None
                else None
            )
            running_mean = (
                torch.tensor(module.running_mean.detach(), device="cuda")
                if module.track_running_stats and module.running_mean is not None
                else None
            )
            running_var = (
                torch.tensor(module.running_var.detach(), device="cuda")
